...
if 'DIRECT' in sys.argv:
	DIRECT=True
if not BEATSONLY and not noshiftgatespec and not nodirb:
	DIRECT=True
		...
		for f in range(self.forecast_length):
			...
			if not BEATSONLY:
				...
				if not NOGATE:
					...
					if RESTRICT:
						outS[bothinds,f]=self.lin(-F.relu(lstm_outST.clone()[bothinds,0,:self.units*2])+F.relu(lstm_outSCT.clone()[bothinds,0,:self.units*2])).view(-1)
					else:
...
def makedata(totallength,sub):
	...
	if SIM:
		...
		print('loading '+SIMSUB+' 1 no snack X5 delay')
